    ...
    public function validate_strong_password( $errors, $user_data ) {
        ...
        if ( ! $password_ok && is_wp_error( $errors ) ) {
            $errors->add(
                ...
                apply_filters(
                    'error_message',
                    __( 'Salasanan tulee olla vähintään 12 merkkiä pitkä, maksimissaan 128 merkkiä.', 'tms-theme-muumib2b' )
                )
            );
        }
        ...
        if ( Security::check_password_pwnd_status( $password ) ) {
            $errors->add(
                ...
                apply_filters(
                    'error_message',
                    __( 'Valittu salasana on vaarantunut. Valitse toinen salasana.', 'tms-theme-muumib2b' )
                )
            );
        }
        else {
